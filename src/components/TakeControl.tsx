...
export default function TakeControl() {
  ...
  const talk = useCallback(() => {
    ...
    speak("അമ്മേ, ദുബായിൽ നിന്ന് മകൻ വിളിക്കുന്നു.");
    later(() => push("Malayalam greeting played: “അമ്മേ, ദുബായിൽ നിന്ന് മകൻ വിളിക്കുന്നു.”"), 600);
  }, [later, phase, push, speak]);
  ...
  return (
    <section id="control" data-testid="control" aria-labelledby="control-h" className="bg-navy text-cream py-14">
      <div className="max-w-6xl mx-auto px-4">
        ...
        <p className="mt-2 text-cream/70">No instructions needed. Press Start check-in and follow the gentle prompts.</p>

        <div className="mt-6 card-warm !bg-white text-navy overflow-hidden">
          <div className="bg-navy-deep text-cream px-5 py-3 flex items-center justify-between">
            <p className="font-semibold" data-testid="control-room-label">[ Parent&apos;s Home — {room} ]</p>
            <p className="text-xs flex items-center gap-2" data-testid="control-phase">
          ...
          <div className="grid md:grid-cols-[1.3fr_1fr]">
            <div className="p-4">
              <div className="relative rounded-2xl overflow-hidden border border-navy/10 bg-cream" data-testid="home-map" role="img" aria-label={`Simulated Kerala home, robot in ${room}`}>
                <svg viewBox="0 0 400 300" className="w-full h-auto">
                  ...
                  <g transform={`translate(${(pos.x / 100) * 400},${(pos.y / 100) * 300})`}>
                    ...
                    <circle r="10" fill={privacy ? "#666" : "#ff6b4a"} data-testid="robot-dot" />
                    <circle r="4" fill="#ffc75f" />
                  </g>
                </svg>
                <div className="absolute top-3 left-3 right-3 flex gap-2 justify-center">
                  {ROOMS.map((r, i) => (
                    <button key={r} data-testid={`room-${i}`} onClick={() => setRoomIdx(i)}
                      className={`text-xs px-2.5 py-1.5 rounded-full font-semibold border ${i === roomIdx ? "bg-navy text-cream" : "bg-white/90"}`}>
                      {r}
                    </button>
                  ))}
                </div>
                {phase === "talking" && (
                  <div className="absolute bottom-3 left-3 right-3 bg-navy/90 text-cream rounded-xl p-3 flex items-center gap-3" data-testid="parent-card">
                    ...
                    <div><p className="font-semibold text-sm">Amma is here</p><p className="text-xs text-cream/70">അമ്മേ, ദുബായിൽ നിന്ന് മകൻ വിളിക്കുന്നു.</p></div>
                  </div>
                )}
                {privacy && (
              ...
              <div className="mt-3 flex flex-wrap gap-2" role="group" aria-label="Call controls">
                ...
                <button data-testid="btn-privacy" aria-pressed={privacy} onClick={() => { setPrivacy((v) => !v); setPhase((p) => (!privacy ? "privacy" : p === "privacy" ? "live" : p)); push(!privacy ? "Privacy mode on — video unavailable." : "Privacy mode off — available again."); }} className="px-3 py-2 rounded-full border text-sm font-semibold min-h-11"><ShieldCheck size={15} className="inline mr-1" aria-hidden /> Privacy</button>
              </div>
            </div>

            <div className="p-4 border-t md:border-t-0 md:border-l border-navy/10">
              <div className="grid grid-cols-3 gap-2 w-44 mx-auto" role="group" aria-label="Drive controls" data-testid="joystick">
                <span />
              ...
              <p className="text-center text-xs text-navy/60 mt-2">Drive with buttons, arrow keys, or drag the dot. Elder needs no controls.</p>
